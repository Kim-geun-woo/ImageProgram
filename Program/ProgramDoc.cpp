...
// ProgramDoc.cpp: CProgramDoc 클래스의 구현
//
...
CProgramDoc::CProgramDoc() noexcept
{
...
// 축소판 그림을 지원합니다.
void CProgramDoc::OnDrawThumbnail(CDC& dc, LPRECT lprcBounds)
{
...
// 검색 처리기를 지원합니다.
void CProgramDoc::InitializeSearchContent()
{
...
void CProgramDoc::SetSearchContent(const CString& value)
{
	if (value.IsEmpty())
	{
...
BOOL CProgramDoc::OnOpenDocument(LPCTSTR lpszPathName)
{
...
BOOL CProgramDoc::OnSaveDocument(LPCTSTR lpszPathName)
{
	CFile File; // 파일 객체 선언
...
void CProgramDoc::OnDownSampling()
{
	int i, j;
	CDownSampleDlg dlg;
...
void CProgramDoc::OnQuantization()
{
...
void CProgramDoc::OnSumConstant()
{
...
void CProgramDoc::OnSubConstant()
{
	CConstantDlg dlg;
...
void CProgramDoc::OnMulConstant()
{
	CConstantDlg dlg;
...
void CProgramDoc::OnDivConstant()
{
	CConstantDlg dlg;
...
void CProgramDoc::OnNegaTransform()
{
	int i;
...
void CProgramDoc::OnGammaCorrection()
{
	CConstantDlg dlg;
...
void CProgramDoc::OnBinarization()
{
	CConstantDlg dlg;
...
void CProgramDoc::OnStressTransform()
{
...
void CProgramDoc::OnHistoStretch()
{
	int i;
...
void CProgramDoc::OnEndInSearch()
{
	int i;
	unsigned char LOW, HIGH, MAX, MIN;
...
double** CProgramDoc::OnMaskProcess(unsigned char* Target, double Mask[3][3])
{ // 회선 처리가 일어나는 함수
	int i, j, n, m;
...
double** CProgramDoc::OnScale(double** Target, int height, int width)
{ // 정규화를 위한 함수
	int i, j;
	double min, max;
...
double** CProgramDoc::Image2DMem(int height, int width)
{ // 2차원 메모리 할당을 위한 함수
	double** temp;
	int i, j;
...
void CProgramDoc::OnGaussianFilter()
{
	int i, j;
...
void CProgramDoc::OnLpfSharp()
{
	CConstantDlg dlg; // 상수를 입력 받기 위한 대화상자 선언
	...
	if (dlg.DoModal() == IDOK) {
		alpha = (int)dlg.m_Constant; // 대화상자를 이용하여 상수를 입력 받음
	}
...
void CProgramDoc::OnDiffOperatorHor()
{
	int i, j;
...
void CProgramDoc::OnHomogenOperator()
{
	int i, j, n, m;
...
double CProgramDoc::DoubleABS(double X)
{// 실수의 절대 값 연산 함수
	if (X >= 0)
		return X;
	else
...
void CProgramDoc::OnBubleSort(double* A, int MAX)
{ // 데이터의 정렬을 처리하는 함수
	int i, j;
	for (i = 0; i < MAX; i++) {
		for (j = 0; j < MAX - 1; j++) {
...
void CProgramDoc::OnSwap(double* a, double* b)
{ // 데이터 교환 함수
	double temp;
	temp = *a;
	*a = *b;
